...
1. Generate detailed notes strictly in the specified target language (${language}).
2. **Ensure that the notes are composed exclusively in the target language (${language}), using only the native alphabets and characters of that language. No words, phrases, or alphabets from any other language are allowed in the output.**
...
- The entire output, including all headings and content, must be strictly in the target language (${language}) and written exclusively using its native alphabets and characters.
- No English words, phrases, or Roman alphabets should appear anywhere in the output.
- Any necessary technical terms should be translated to their native language equivalent, or transliterated using native script if no translation exists.
